...
class Gui():    
    ...
    def asignar_turno(self):
        if not self.tv_pacientes.selection():
            ...
            p = self.gestion._buscar_por_id(id)
    ...
    def boton_ok2(self):
        elem = self.tv_pacientes.selection()
        id = int(self.tv_pacientes.item(elem)['text'])
        self.gestion.asignar_turno(id, self.a_fecha.get(), self.a_hora.get(), self.a_desc.get())
        self.tv_turnos.insert("", END, text=paciente.id, values=(paciente.apellido, self.a_fecha.get(), self.a_hora.get(), self.a_desc.get()))
    ...
    def boton_ok3(self, event=None, turnos=None):
        ...
        for turno in turnos:
            self.tv_turnos.insert("", END, text=paciente.id, values=(paciente.apellido, turno.fecha_turno, turno.hora, turno.descripcion), iid=turno.id)
